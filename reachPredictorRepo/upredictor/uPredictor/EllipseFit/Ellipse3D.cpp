...
<<<<<<< HEAD
	return	cart3DRotMatToHomRT
		(_eif.getRotToXY().transpose()) 
		*	twoDHomToThreeDHom(_ellipse.getPointAtThetasH
		  (thetas), -_eif.getPlane().getPlaneN()(3));
=======
	return	cart3DRotMatToHomRT(_eif.getRotToXY().transpose()) *
...
//cout << _ellipse.getRT2D().getCX() << endl;
//cout << _ellipse.getRT2D().getCY() << endl;
//cout << _ellipse.getRT2D().getTheta() << endl;
//cout << _ellipse.getA() << " , " << _ellipse.getB() << endl;
...
<<<<<<< HEAD
	_totalXYCost = _XYCosts.sum()/double(_XYCosts.rows());
	return thetas;
}


MatrixXd Ellipse3D::ellipticalInterpolator
 (size_t sliceN){ 
